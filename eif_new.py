...
class iForest(object):
    ...
    def __init__(
        ...
        for _ in range(
            ...
            ix = np.random.choice(range(self.nobjs), self.sample)
            X_p = X[ix]
    ...
    def compute_paths(self, X_in):
        ...
        if self.n_jobs != 1:
            warnings.warn(
                "You are calling the unparallelized version of the function but set n_jobs={}. For parallelized version use compute_paths_parallel.".format(
                    self.n_jobs
                )
            )
        S = np.zeros(len(X_in))
    ...
    def compute_paths_parallel(self, X_in):
        """
        compute_paths(X_in)
        Compute anomaly scores for all data points in a dataset X_in
...
def compute_path(ntrees, X, c, Trees):
    ...
    for i in range(len(X)):
        ...
        for j in range(ntrees):
            h_temp += (
                PathFactor(X[i], Trees[j]).path * 1.0
            )  # Compute path length for each point
        Eh = (
            h_temp / ntrees
        )  # Average of path length travelled by the point in all trees.
        S_i = 2.0 ** (-Eh / c)  # Anomaly Score
        sub_S.append(S_i)
    return sub_S
