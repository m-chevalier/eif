...
Node *iTree::add_node(double *X_in, int size_in, int e_in, RANDOM_ENGINE &random_engine_in)
{
	...
	else
	{
		...
		for (int i = 0; i < dim; i++)
		{
			...
			{
				int index = i + j * dim;
				if (Xmins[i] > X_in[index])
					Xmins[i] = X_in[index];
				if (Xmaxs[i] < X_in[index])
					Xmaxs[i] = X_in[index];
			}
		}
		...
		for (int i = 0; i < size_in; i++)
		{
			...
			{
				for (int j = 0; j < dim; j++)
					XL.push_back(X_in[j + index]);
				sizeXL += 1;
			}
			else
			{
				for (int j = 0; j < dim; j++)
					XR.push_back(X_in[j + index]);
				sizeXR += 1;
			}
		}
...
iForest::iForest(int ntrees_in, int sample_in, int limit_in = 0, int exlevel_in = 0, int random_seed_in = -1, bool parallel_in=1)
{
	...
	limit = limit_in;
	if (limit_in <= 0)
		limit = (int)ceil(log2(sample)); // limit must be a positive integer
	exlevel = exlevel_in;
	c = c_factor(sample);
